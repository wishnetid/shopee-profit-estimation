...
type StandardFeeBreakdown = {
  administration: number;
  orderProcessing: number;
  freeShippingXtra: number;
  promoXtra: number;
  premium: number;
};
...
type EstimationOrder = {
  ...
  voucherPhysicalTotal: number | null;
  voucherDeduped: boolean;
  estimatedShopeeFees: number | null;
  standardFees: StandardFeeBreakdown | null;
...
type DailyRow = {
  ...
  standardFeeBreakdown: StandardFeeBreakdown;
...
const REASON_COPY: Record<string, string> = {
  ...
  VOUCHER_PENJUAL_TIDAK_KONSISTEN: 'Nominal voucher penjual antar-SKU tidak konsisten',
};
...
const standardFeeRows = (breakdown: StandardFeeBreakdown) => [
  ['Administrasi', '8,25%', breakdown.administration],
  ['Proses Pesanan', 'Rp1.250/order', breakdown.orderProcessing],
  ['Gratis Ongkir XTRA', '5,00%', breakdown.freeShippingXtra],
  ['Promo XTRA', '4,50%', breakdown.promoXtra],
  ['Premium', '0,50%', breakdown.premium],
] as const;

function FeeBreakdownDetails({ breakdown }: { breakdown: StandardFeeBreakdown }) {
  return <details className="mt-1 text-left text-xs text-slate-600"><summary className="cursor-pointer select-none text-purple-700 hover:text-purple-900">Rincian rate</summary><dl className="mt-1.5 grid gap-1 rounded-md bg-slate-50 p-2 text-[11px] leading-4 text-slate-600">{standardFeeRows(breakdown).map(([label, rate, amount]) => <div key={label} className="flex items-baseline justify-between gap-3"><dt>{label} <span className="text-slate-400">{rate}</span></dt><dd className="whitespace-nowrap font-semibold tabular-nums">{formatIdr(amount)}</dd></div>)}</dl></details>;
}

function StandardFeeBreakdown({ breakdown }: { breakdown: StandardFeeBreakdown }) {
  return <section className="mb-5 rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-950 lg:p-5"><div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between"><div><h2 className="font-semibold">Rincian Estimasi Potongan Standar Shopee</h2><p className="mt-1 text-xs leading-5 text-rose-900">Forecast rate standar dari basis order setelah voucher penjual. Bukan potongan settlement aktual.</p></div><span className="text-xs font-medium text-rose-800">Tidak termasuk AMS, Ads, PPN, retur, atau adjustment</span></div><dl className="mt-4 grid gap-x-6 gap-y-3 sm:grid-cols-2 xl:grid-cols-5">{standardFeeRows(breakdown).map(([label, rate, amount]) => <div key={label} className="border-t border-rose-200 pt-2"><dt className="text-xs font-semibold text-rose-800">{label} <span className="font-normal">({rate})</span></dt><dd className="mt-1 text-sm font-bold tabular-nums">{formatIdr(amount)}</dd></div>)}</dl></section>;
}
      ...
      {tab === 'orders' ? <ProfitActualPanel storeId={storeId} view="orders" /> : tab === 'returns' ? <ProfitActualPanel storeId={storeId} view="returns" /> : tab === 'reconciliation' ? <SettlementBalanceReconciliationPanel storeId={storeId} /> : tab === 'balanceAnalysis' ? <MyBalanceAnalysisPanel storeId={storeId} /> : <>
        ...
        {data && !loading && <>
          <section className="mb-5"><div className="mb-2 flex items-center justify-between"><h2 className="text-sm font-semibold text-slate-800">Forecast Order Terfilter</h2><span className="text-xs text-slate-500">Order.all + potongan standar + HPP</span></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <SummaryCard label="Estimasi Potongan Standar Shopee" value={formatIdr(data.summary.estimatedStandardShopeeFees)} detail="Rate standar; bukan potongan settlement aktual" tone="rose" />
          ...
          <StandardFeeBreakdown breakdown={data.summary.standardFeeBreakdown} />
