...
const REASON_COPY: Record<string, string> = {
  ...
  TOTAL_PEMBAYARAN_TIDAK_KONSISTEN: 'Total pembayaran item tidak konsisten',
  TOTAL_PEMBAYARAN_TIDAK_VALID: 'Total pembayaran tidak valid',
};
...
function SummaryCard({ label, value, detail, tone }: { label: string; value: string; detail: string; tone: 'purple' | 'rose' | 'indigo' | 'amber' | 'slate' }) {
  ...
  return (
    <section className={`rounded-xl border p-4 ${colors[tone]}`}>
      ...
      <p className="mt-2 text-xl font-bold tabular-nums lg:text-2xl">{value}</p>
...
export default function ProfitPage() {
  ...
  return (
    <ProfitEstimationContent
      key={storeId || 'no-store'}
      storeId={storeId}
      activeStoreName={activeStore?.store_name || 'toko aktif'}
    />
  );
}

function ProfitEstimationContent({
  storeId,
  activeStoreName,
}: {
  storeId: string;
  ...
  const load = useCallback(async (nextPage = 1, nextLimit = limit) => {
    ...
    try {
      ...
      if (requestId === requestSequence.current) {
        setError(caught instanceof Error ? caught.message : 'Gagal memuat estimasi kotor.');
      }
    } finally {
  ...
  const totalPages = useMemo(
    () => data ? Math.max(1, Math.ceil(data.orders.total / data.orders.limit)) : 1,
    [data],
  );
    ...
    <div className="p-4 lg:p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <span className="inline-flex w-fit rounded-full border border-purple-200 bg-purple-50 px-3 py-1.5 text-xs font-semibold text-purple-700">
            Toko aktif: {activeStoreName}
          </span>
        </div>
        ...
        <div className="mb-5 flex gap-2 border-b border-slate-200">
          <button
            ...
            className={`border-b-2 px-3 py-2 text-sm font-semibold ${tab === 'estimate' ? 'border-purple-600 text-purple-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Estimasi Kotor
          </button>
          <button
            ...
            className={`border-b-2 px-3 py-2 text-sm font-semibold ${tab === 'actual' ? 'border-purple-600 text-purple-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Profit Aktual
          </button>
        </div>
        ...
        {tab === 'actual' ? (
          <section className="rounded-xl border border-slate-200 bg-white p-5 lg:p-6">
            <div className="flex items-start gap-3">
              <div className="rounded-lg bg-slate-100 p-2 text-slate-600"><LockKeyhole className="h-5 w-5" /></div>
              <div>
                <h2 className="font-semibold text-slate-900">Profit Aktual — Belum Tersedia</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                  Fase ini menunggu kontrak settlement `Penghasilan / Order`, return/refund, QC stok, serta biaya eksternal.
                  Estimasi Kotor di tab sebelumnya bukan pengganti angka profit aktual.
                </p>
              </div>
            </div>
          </section>
          ...
          <>
            <section className="mb-5 rounded-xl border border-purple-200 bg-purple-50 p-4 lg:p-5">
              <div className="flex items-start gap-3">
            ...
            <section className="mb-5 rounded-xl border border-slate-200 bg-white p-4 lg:p-5">
              <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end">
                <label className="text-sm font-medium text-slate-700">
                  Dari tanggal <span className="font-normal text-slate-400">(opsional)</span>
                  <input
                    type="date"
                    value={dateFrom}
                    onChange={(event) => updateDate('from', event.target.value)}
                    className="mt-1.5 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-100"
                  />
                </label>
                <label className="text-sm font-medium text-slate-700">
                  Sampai tanggal <span className="font-normal text-slate-400">(opsional)</span>
                  <input
                    type="date"
                    value={dateTo}
                ...
                <button
                  type="button"
                  onClick={() => void load(1, limit)}
                  disabled={!storeId || loading}
                  className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-purple-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />
                  {loading ? 'Memuat…' : 'Muat Estimasi'}
                </button>
                <button
                  type="button"
                  onClick={resetResult}
                  disabled={!data && !error && !loading}
                  className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Reset Hasil
                </button>
              </div>
              <p className="mt-3 text-xs leading-5 text-slate-500">
                Tidak ada pengecekan otomatis. Ubah rentang lalu tekan Muat Estimasi. Rentang kosong membaca seluruh data aktif toko.
              </p>
            </section>
              ...
              <>
                ...
                <div className="mb-5 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <span>
                      Scope: <b className="text-slate-800">{data.dateRange.dateFrom ? formatDate(data.dateRange.dateFrom) : 'semua tanggal'}</b>
                      {data.dateRange.dateTo && <> s.d. <b className="text-slate-800">{formatDate(data.dateRange.dateTo)}</b></>}
                    </span>
                    <span>{data.summary.excludedOrderCount} order tidak eligible · {data.summary.adsDuplicateEventCount} event Ads overlap dilewati</span>
                  </div>
                  {data.skuImport ? (
                    <p className="mt-2 text-xs text-slate-500">HPP memakai Master SKU import terbaru: {data.skuImport.sourceFile} · {formatDate(data.skuImport.importedAt)}.</p>
                  ) : (
                    <p className="mt-2 text-xs font-medium text-amber-700">Master SKU belum tersedia. Semua order yang memerlukan HPP akan ditandai HPP Belum Lengkap.</p>
                  )}
                </div>
                    ...
                    <table className="w-full min-w-[1010px] text-sm">
                      <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-right">Order Estimasi</th>
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {data.daily.length === 0 ? (
                          ...
                          <tr key={row.date} className="hover:bg-slate-50">
                            ...
                            <td className="px-4 py-3 text-right tabular-nums">{row.estimatedOrderCount}</td>
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                  <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-4 lg:flex-row lg:items-center lg:justify-between lg:px-5">
                    <div>
                      <h2 className="font-semibold text-slate-900">Estimasi Per Order</h2>
                      <p className="mt-1 text-sm text-slate-500">Termasuk status HPP belum lengkap, review, dan tidak eligible agar ada audit trail.</p>
                    </div>
                    <label className="text-xs font-semibold text-slate-600">
                      ...
                      <select
                        ...
                        className="ml-2 rounded-md border border-slate-300 bg-white px-2 py-1.5 text-sm font-normal text-slate-800"
                      >
                    ...
                    <table className="w-full min-w-[1120px] text-sm">
                      <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                        <tr>
                          ...
                          <th className="px-4 py-3">Status Shopee</th>
                          <th className="px-4 py-3">Status Estimasi</th>
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {data.orders.data.length === 0 ? (
                          <tr><td colSpan={9} className="px-4 py-10 text-center text-slate-400">Tidak ada order pada rentang ini.</td></tr>
                        ) : data.orders.data.map((order) => (
                          <tr key={order.no_pesanan || `${order.orderDate}-${order.itemCount}`} className="align-top hover:bg-slate-50">
                            <td className="whitespace-nowrap px-4 py-3">{formatDate(order.orderDate)}</td>
                            <td className="whitespace-nowrap px-4 py-3 font-medium text-slate-900">{order.no_pesanan || '—'}</td>
                            <td className="px-4 py-3">{order.statusPesanan || '—'}</td>
                            <td className="px-4 py-3"><StatusBadge status={order.estimationStatus} /></td>
                            <td className="px-4 py-3 text-right tabular-nums">{order.itemCount}</td>
                            <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums">{formatIdr(order.totalPembayaran)}</td>
                            <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums">{formatIdr(order.totalHpp)}</td>
                            <td className="whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums text-purple-700">{formatIdr(order.estimasiKotor)}</td>
                            <td className="min-w-64 px-4 py-3 text-xs leading-5 text-slate-500">{reasonText(order.reasons)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3">
                      <button
                        ...
                        onClick={() => void load(page - 1, limit)}
                        className="inline-flex items-center gap-1 rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        <ChevronLeft className="h-4 w-4" /> Sebelumnya
                      </button>
                      <span className="text-xs text-slate-500">Halaman {page} dari {totalPages} · {data.orders.total} order</span>
                      <button
                        type="button"
