...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [balanceRows] = await conn.query<RowDataPacket[]>(`SELECT COALESCE(NULLIF(b.no_pesanan_direct,''),NULLIF(b.no_pesanan_extracted,'')) no_pesanan,DATE_FORMAT(b.transaction_at, '%Y-%m-%d %H:%i:%s') transaction_at,b.type_transaksi,b.jenis_transaksi,b.jumlah_signed,b.description,b.status,b.saldo_akhir,i.source_file,b.source_excel_row FROM balance_transactions_raw b JOIN balance_report_imports i ON i.id=b.balance_report_import_id WHERE i.store_id=? AND COALESCE(NULLIF(b.no_pesanan_direct,''),NULLIF(b.no_pesanan_extracted,'')) IN (${marks})`, [storeId, ...cohortIds]);
