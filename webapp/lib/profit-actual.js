...
function buildProfitActualReport({ orderRows, skuRows, settlementRows, settlementExistenceRows = settlementRows, exceptionOrderNumbers, exceptionEvidenceRows = [], balanceRows = [] }) {
  ...
  summary.unresolvedPcs = orders.filter((order) => !['settled_normal', 'partial_return_provisional', 'full_return_cash_final_negative', 'batal'].includes(order.bucket)).reduce((total, order) => total + order.orderedPcs, 0);
  ...
  summary.unresolvedOrders = orders.filter((order) => !['settled_normal', 'partial_return_provisional', 'full_return_cash_final_negative', 'batal'].includes(order.bucket)).length;
