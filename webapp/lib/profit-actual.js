...
function buildProfitActualReport({ orderRows, skuRows, settlementRows, settlementExistenceRows = settlementRows, exceptionOrderNumbers }) {
  ...
  const orders = [...groups.values()].map((group) => {
    ...
    if (bucket === 'settled_normal') { summary.settledNormal++; summary.settlement += settlement; summary.hpp += totalHpp; summary.profit += profitActual; } else if (bucket === 'completed_unsettled') summary.completedUnsettled++; else if (bucket === 'settlement_outside_release_range') summary.settlementOutsideReleaseRange++; else if (bucket === 'pending') summary.pending++; else if (bucket === 'exception' || bucket === 'hpp_issue') { summary.exception++; summary.settlementExcluded += settlement || 0; } else summary.cancelled++;
