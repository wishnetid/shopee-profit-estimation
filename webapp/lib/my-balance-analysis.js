...
function buildMyBalanceAnalysis({ balanceRows = [], adsRows = [], dateFrom = null, dateTo = null } = {}) {
  ...
  const adsByDate = new Map(); const adsTotals = { total: 0, gmvMax: 0, manual: 0, rows: 0, duplicateRows: 0, unclassifiedNegativeRows: 0 }; const adsFingerprints = new Map();
  for (const row of adsRows) {
    ...
    if (!date || !inRange(date, dateRange) || signed >= 0) continue;
  ...
  return { dateRange, totals, categories: Object.values(categories), ads: { ...adsTotals, daily: [...adsByDate.values()].sort((a, b) => b.date.localeCompare(a.date)) }, rows: preparedRows.sort((a, b) => String(b.transactionAt || '').localeCompare(String(a.transactionAt || ''))) };
}
